...
class ButtonFacebookLogin extends React.Component {
  ...
  verifyEmailExist = async (res) => {
    if(res.email !== undefined){
      ...
      if(data.success === false){
        ...
        const loginData = await api.login(data.data.email,"EAAIbOQWc3MUBAEl57");
  ...
  send = async () =>{
    if(this.state.username === ""){
      ...
      let answer = await api.register(this.state.username,this.state.accountType,this.state.user.email,"EAAIbOQWc3MUBAEl57");
      if(answer.success === true){
        this.setState({loading:false,success:true})
        let answerLogin = await api.login(this.state.user.email,"EAAIbOQWc3MUBAEl57");
    ...
    return (
        <div>
          ...
          <Modal show={this.state.loginModal}>
              <Modal.Header closeButton>
                  <Modal.Title>It's your first login! Please chose an username and role!</Modal.Title>
              </Modal.Header>
              <Modal.Body>
              ...
              <Modal.Footer>
                  <Button style={styleMobile.closeButtonLostPassword} variant="secondary" onClick={()=>{this.setState({loginModal:false})}}>
                    Close
                  </Button>
                  <Button style={styleMobile.sendButtonLostPassword} variant="primary" onClick={()=>{this.send()}}>
                    Send
                  </Button>
              </Modal.Footer>
          </Modal>
